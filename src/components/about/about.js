import { useEffect, useState } from "react";
import { getCityAbout } from "../../services/dashboard/dashboard.service";
import "./about.css";

export default function About({ city }) {
  const [About, setAbout] = useState([]);
  useEffect(() => {
    getCityAbout(city)
      .then((res) => {
        console.log(res);
        setAbout(res.data);
      })
      .catch((err) => {
        console.log("rest error", err);
      }); // eslint-disable-next-line
  }, [city]);
  return (
    <>
      <div>
        <div className="city_img">
          <img
            className="img_tag"
            src="https://www.karnatakatourism.org/wp-content/uploads/2020/05/p25-gallery.jpg"
          />
        </div>
        <h1 className="city_tagline">{About.city_desc}</h1>
        <img
          src="https://storage.googleapis.com/download/storage/v1/b/may-cityguide/o/kitten.png?generation=1653037016118901&alt=media"
          alt="cat"
        />
        <img
          src="https://storage.googleapis.com/may-cityguide/kitten.png?X-Goog-Algorithm=GOOG4-RSA-SHA256&X-Goog-Credential=hu20-gcpserviceaccount1%40us-gcp-ame-con-116-npd-1.iam.gserviceaccount.com%2F20220520%2Fauto%2Fstorage%2Fgoog4_request&X-Goog-Date=20220520T112439Z&X-Goog-Expires=900&X-Goog-SignedHeaders=host&X-Goog-Signature="
          alt="cat_authurl"
        />
        <img
          src="https://storage.googleapis.com/download/storage/v1/b/may-cityguide/o/rtt%2Fkitten.png?generation=1653053670245000&alt=media"
          alt="new media link"
        />
        <img
          src="https://storage.cloud.google.com/may-cityguide/rtt/kitten.png"
          alt="new auth link"
        />
      </div>
    </>
  );
}
