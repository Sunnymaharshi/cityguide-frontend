import { useEffect, useState } from "react";
import { motion, AnimatePresence } from "framer-motion";
import Accordion from "../accordion/Accordion";
import "./metro.css"
import { getMetro } from "../../services/dashboard/dashboard.service";
import green_schedule from "../../assets/green_line_schedule.png";
import purple_schedule from "../../assets/purple_line_schedule.png";
const tabs=["Map", "Lines", "Schedule"];

const Map=(data)=> {

  return (
      <div className="mapimg-div">
        <img className="mapdata" src="https://lh5.googleusercontent.com/ca68mb6uJ0rRcZobn9QWKzFETqc4vlvNjGslMu_3G6aYm-HJRCupYguhrV4TTfx4eUDNSQv7epFXFNxzCiBzQuZyKFac6nIqSYpzu1_e3NkdAi-_2ErzWxbGzy4YGMoRS8R8Ueei" alt="no data"/>
      </div>
  );
}
const Lines=(data1)=>{
  
    // console.log(data1.data.metromap_img);
  //const str1=JSON.stringify(data1.data.metromap_img);
 // console.log(str1);
  const [expanded, setExpanded] = useState();
  const [lines, setLines] = useState([{title: "Green line",img:"https://upload.wikimedia.org/wikipedia/en/thumb/5/5a/NammaMetro-GreenLine.jpg/500px-NammaMetro-GreenLine.jpg"}, 
  {title: "Purple line",img:"data:image/png;base64,"}])
  return lines.map((line,i) => (
    <Accordion i={i} expanded={expanded} setExpanded={setExpanded} line={line} key={i}/>
  ));
}
const Schedule=(data)=>{
  const [expanded, setExpanded] = useState();
  const [lineschedule, setLineschedule] = useState([{title: "Green line",img:green_schedule}, {title: "Purple line",img:purple_schedule}])
  return lineschedule.map((line,i) => (
    <Accordion i={i} expanded={expanded} setExpanded={setExpanded} line={line} key={i}/>
  ));
}
export default function Metro({city}){

    const [selectedTab, setSelectedTab] = useState(tabs[0]);
    const [data, setdata] = useState([]);
    useEffect(() => {
      getMetro(city).then(res=>{
        console.log(res);
        setdata(res.data);
      });
    
      
    }, [city])
    

  return (
    <div className="window">
      <nav>
        <ul className="ul">
          {tabs.map((item) => (
            <li
              key={item}
              className={item === selectedTab ? "selected" : ""}
              onClick={() => setSelectedTab(item)}
            >
              { item }

              {item === selectedTab ? (
                <motion.div className="underline" layoutId="underline" />
              ) : null}
            </li>
          ))}
        </ul>
      </nav>
      <main>
        <AnimatePresence exitBeforeEnter>
          <motion.div
            key={selectedTab ? selectedTab.label : "empty"}
            className="metro-content"
            animate={{ opacity: 1, y: 0 }}
            initial={{ opacity: 0, y: 20 }}
            exit={{ opacity: 0, y: -20 }}
            transition={{ duration: 0.15 }}
          >
            {selectedTab==="Map" && <Map data={data[0]}/> }
            {selectedTab==="Lines" && <Lines data={data[1]}/> }
            {selectedTab==="Schedule" && <Schedule data={data}/> }
          </motion.div>
        </AnimatePresence>
      </main>
    </div>
  );

}